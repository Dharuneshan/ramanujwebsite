...
const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    cardsRef.current.forEach((card, i) => {
      ...
      const newTransform = {
        translateY: Math.round(translateY * 10) / 10, // Reduced precision to prevent micro-vibrations
        scale: Math.round(scale * 100) / 100, // Reduced precision
        rotation: Math.round(rotation * 10) / 10, // Reduced precision
        blur: Math.round(blur * 10) / 10 // Reduced precision
      };
      ...
      const hasChanged =
        ...
        Math.abs(lastTransform.scale - newTransform.scale) > 0.01 || // Increased threshold
  ...
  const setupLenis = useCallback(() => {
    if (useWindowScroll) {
      const lenis = new Lenis({
        duration: 1.0, // Reduced duration for smoother feel
        easing: t => Math.min(1, 1.001 - Math.pow(2, -10 * t)),
        smoothWheel: true,
        touchMultiplier: 1.5, // Reduced for less aggressive touch
        infinite: false,
